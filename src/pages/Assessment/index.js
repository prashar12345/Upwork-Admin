...
const Assessments= (p) => {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`assessment?id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this Assessment`)){
            loader(true)
            ApiClient.put(`change/status?model=assesments&id=${itm._id}&status=${status}`).then(res=>{
                if(res.success){
                    getData()
                    toast.success(`Assessment ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
    ...
    const edit=(id)=>{
        history.push(`/AddEditAssessment/${id}`)
    }
