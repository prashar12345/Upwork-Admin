        ...
        <Layout>
            <div className="d-flex justify-content-between align-items-center">
                ...
                <article className="d-flex">
                    ...
                    {isAllow('addAssessment') ? <Link className="btn btn-primary mr-2" to="/AddEditAssessment">
                        Add  Assessment 
                    </Link> : <></>}
                        ...
                        <table class="table table-striped">
                            <thead className='table_head'>
                                <tr className='heading_row'>
                                    <th scope="col" className='table_data'> Assessment Name</th>
                                   
                                    <th scope="col" className='table_data'>Weightage Per Question</th>
                                    <th scope="col" className='table_data'>Minimum Passing Score</th>
                                    <th scope="col" className='table_data'>Skills</th>
                         ...
                         {data.map((itm,index)=>(      <tr className='data_row'>
                                        ...
                                        <td className='table_dats' onClick={e => edit(itm.id)}>{itm.weightagePerQuestion}</td>
                                        <td className='table_dats' onClick={e => edit(itm.id)}>{itm.passingScrore}</td>
                                        <td className='table_dats' onClick={e => edit(itm.id)}>{itm.skills&&itm.skills.map((item)=><span>{item.name},</span>)}</td>
                                        ...
                                        <td className='table_dats'>
                                            <div className="action_icons">
                                                ...
                                                {isAllow('deleteAssessment') ? <span className='edit_icon' onClick={() => deleteItem(itm.id)}>
...

            {
                !loaging && total > filters.count ? <div className='paginationWrapper'>
                    <span>Show {filters.count} from {total} Assessments</span>
                    <Pagination
