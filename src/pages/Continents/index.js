...
const Continent = (p) => {
    ...
    useEffect(() => {
        let cols = []
        for (let i = 0; i <= 1; i++) {
            cols.push(userTableModel.Continents[i])
        }
        setTableCols(cols)
    }, [])
    ...
    const getData = (p = {}) => {
        ...
        // Please Add the Api before making it live
        ApiClient.get('api/continent/listing', filter).then(res => {
            if (res.success) {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            //  Enter the delete api befiore making it live
            ApiClient.delete('api/continent/delete', {id: id }).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this`)){
            loader(true)
            ApiClient.put(`api/continent/status/change`,{id:itm.id,status}).then(res=>{
                if(res.success){
                    getData()
                }
                loader(false)
    ...
    const edit=(id)=>{
        history.push(`/continents/edit/${id}`)
    }
    const exportfun=async()=>{
        const token=await localStorage.getItem("token");
        const req = await axios({
            method: "get",
            url: `${environment.api}api/continent/export`,
            responseType: "blob",
            body:{token:token}
          });
