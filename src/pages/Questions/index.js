...
const QuestionsLists = (p) => {
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('assessments/frontend', filter).then(res => {
            if (res.success) {
                const Newdata=res.data;
                const array=[];
                Newdata.map((item,index)=>{
   if(item.id==id){
    setData(item.assessmentQuestions)
    setTotal(item.assessmentQuestions&&item.assessmentQuestions.length)
   }
                })
                // setData(res.data)
            }
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`assessment/question?id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this Skill Type`)){
            loader(true)
            ApiClient.put(`change/status?model=skilltype`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
                    toast.success(`Skill Type ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
    ...
    const edit=(id)=>{
        history.push(`/AddEditquestions/${id}`)
    }
