...
const Coupons = (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 50, search: '', dateFrom: '', dateTo: '' })
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('api/coupon/listing', filter).then(res => {
            if (res.success) {
    ...
    const reset = () => {
        let filter={
            page: 1, count: 50,dateFrom: '', dateTo: '', status: ''
        }
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete('api/coupon/delete', { id: id }).then(res => {
                if (res.success) {
    ...
    const statusChange = (itm) => {
        ...
        if (window.confirm(`Do you want to ${status == 'active' ? 'Activate' : 'Deactivate'} this`)) {
            loader(true)
            ApiClient.put(`api/coupon/update`, { id: itm.id, status }).then(res => {
                if (res.success) {
                    getData()
                }
                loader(false)
    ...
    const exportfun = async () => {
        ...
        const req = await axios({
            method: "get",
            url: `${environment.api}api/export/coupons`,
            responseType: "blob",
            body: { token: token }
        });
