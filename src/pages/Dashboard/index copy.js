...
const Dashboard = () => {
  ...
  const isInterval = (arr) => {
    // console.log("arr[0].daysno", arr[0].daysno)
    let interval = 0
    if (arr && arr.length && arr[0].daysno < 6) interval = 1
    if (filters.type == 'week') interval = 0
    return interval
  }
    ...
    ApiClient.get("analytics", payload).then(res => {
      if (res && res.success) {
        const setObj = (itm, noWeek = false) => {
          ...
          if (filter.type == 'week') {
            obj = {
              x: new Date(itm.date ? itm.date : itm.months),
        ...
        totalUserAccounts = [...totalUserAccounts, { x: new Date(), y: 0 }]
        ...
        setTimeout(() => {
          chartModel.setChart({ id: 'totalAccCreated', data: sortby(totalUserAccounts.length ? totalUserAccounts : defaultCart, filter), type: 'line' })
    ...
    ApiClient.get('assessment/analytics', filter).then(res => {
      ...
      if (levarr.length) {
        levarr.sort(function (a, b) {
          return new Date(a.date ? a.date : a.months) - new Date(b.date ? b.date : b.months);
        })
        levarr.map((itm, i) => {
          ...
          if (filter.type == 'week') prm.x = new Date(itm.date)
      ...
      console.log("ulevkeys", ulevkeys)
      console.log("levmkeys", levmkeys)
