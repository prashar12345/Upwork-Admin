...
const Regions = (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 50, search: '', currencyId: '', continentId: '',countryId:'' })
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('api/region/listing', filter).then(res => {
            if (res.success) {
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete('api/region/delete', { id: id }).then(res => {
                if (res.success) {
    ...
    const statusChange = (itm) => {
        ...
        if (window.confirm(`Do you want to ${status == 'active' ? 'Activate' : 'Deactivate'} this`)) {
            loader(true)
            ApiClient.put(`api/region/status/change`, { id: itm.id, status }).then(res => {
                if (res.success) {
                    getData()
                }
                loader(false)
    ...
    const GetCountry = (p={}) => {
        ApiClient.get(`api/country/listing`,p).then(res => {
    ...
    const exportfun = async () => {
        ...
        link.download = `Region.xlsx`;
        link.click();
    }
    const reset = () => {
        let filter = {
            page: 1, count: 50, search: '', currencyId: '', continentId: '', countryId: ''
        }
