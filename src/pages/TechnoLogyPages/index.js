...
const Technology = (p) => {
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('technologies', filter).then(res => {
            if (res.success) {
                setData(res.data)
                setTotal(res.total)
            }
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`technology?id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this Technology`)){
            loader(true)
            ApiClient.put(`change/status?model=technology`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
                    toast.success(`Technology ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
