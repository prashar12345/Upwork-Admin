...
const CategoryList= (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 50, search: '', type: '' })
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=category&id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this category`)){
            loader(true)
            ApiClient.put(`change/status?model=category`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
