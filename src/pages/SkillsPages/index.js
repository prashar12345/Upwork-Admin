...
const Skills = (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 20, search: '', catType: '',skillType:"" })
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('skills', filter).then(res => {
            if (res.success) {
                setData(res.data)
                setTotal(res.total)
            }
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=skills&id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        ...
        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this Skill`)){
            loader(true)
            ApiClient.put(`change/status?model=skills`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
                    toast.success(` Skill ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
