...
const RolesList = (p) => {
    ...
    const [form, setform] = useState({ role: '', permission: permissions })
    ...
    const openModal = (itm = '') => {
        ...
        if (itm) {
            let pms = permissions
            if (itm.permission) pms = { ...permissions, ...itm.permission }
            setform({ ...form, id: itm.id, role: itm.role, permission: pms })
        }
    ...
    return (
        <Layout>
                    ...
                    <tbody>
                        {!loader && data && data.map((itm, i) => {
                            return <tr key={i}>
                                ...
                                <td>
                                    <a className="linkclass mx-1" onClick={() => openModal(itm)}>Edit Permissions</a>
                                </td>
                            </tr>
                        })}
                    </tbody>
            ...
            <AddEditRole form={form} modalClosed={modalClosed} />
        </Layout>
    );
};
