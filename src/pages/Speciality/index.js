...
const Speciality= (p) => {
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('specialities', filter).then(res => {
...
//   loader(false)
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=speciality&id=${id}`).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        let modal='speciality'
        let status='active'
        if(itm.status=='active') status='deactive'

        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this speciality`)){
            loader(true)
            ApiClient.put(`change/status?model=speciality`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
                    toast.success(`Speciality ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
    ...
    const edit=(id)=>{
        history.push(`/AddEditspeciality/${id}`)
    }
  
    const view=(id)=>{
        history.push("/specialityDetails/"+id)
    }
    const tabChange=(tab)=>{
        setTab(tab)
    }
