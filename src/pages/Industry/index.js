...
const Industry = (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 50, search: '', role:role||'', isDeleted: false })
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=industrytype&id=${id}`,).then(res => {
                if (res.success) {
    ...
    const statusChange=(itm)=>{
        let modal='industrytype'
        let status='active'
        if(itm.status=='active') status='deactive'

        if(window.confirm(`Do you want to ${status=='active'?'Activate':'Deactivate'} this industry type`)){
            loader(true)
            ApiClient.put(`change/status?model=industrytype&id=${itm.id}&status=${status}`,{status,id:itm.id}).then(res=>{
                if(res.success){
                    getData()
                    toast.success(`Industry Type ${status=='active'?'Activated':'Deactivated'} Successfully`)
                }
                loader(false)
            })
        }
    ...
    const blockunblock=(itm)=>{
        if(window.confirm(`Do you want to ${!itm.isBlock?'Block':'Un-block'} this industry type`)){
            loader(true)
            ApiClient.put(`industry`,{id:itm.id,isBlock:itm.isBlock?false:true}).then(res=>{
                if(res.success){
                    getData()
                }
                loader(false)
    ...
    // const view=(id)=>{
    //     history.push("/industryDetail/"+id)
    // }

    const edit=(id)=>{
        let url=`/industry/edit/${id}`
        if(role) url=`/industry/${role}/edit/${id}`
        history.push(url)
    }

    const add=()=>{
        let url=`/industry/add`
        if(role) url=`/industry/${role}/add`
        history.push(url)
    }
