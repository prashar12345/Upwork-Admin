...
{filters.status ||filters.country? <button className="btn btn-primary mr-2" onClick={()=>reset()}>
                        Reset  
                    </button>:<></>}
                    ...
                    <tbody>
                        {!loaging && data && data.map((itm, i) => {
                            return <tr className='data_row'>
                                <td className='table_dats' onClick={e => edit(itm,false)}>{itm.name}</td>
                                <td className='table_dats'>{datepipeModel.date(itm.createdAt)}</td>
                                <td className='table_dats'>{datepipeModel.date(itm.updatedAt)}</td>
                                <td className='table_dats'>{itm.country}</td>
