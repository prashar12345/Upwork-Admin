...
const SubSkills = (p) => {
    ...
    const [filters, setFilter] = useState({ page: 1, count: 20, search: '', catType: '' })
    ...
    const getData = (p = {}) => {
        ...
        ApiClient.get('skill/types', filter).then(res => {
            if (res.success) {
                setData(res.data)
                setTotal(res.total)
            }
    ...
    const deleteItem = (id) => {
        if (window.confirm("Do you want to delete this")) {
            loader(true)
            ApiClient.delete(`delete?model=skilltype&id=${id}`).then(res => {
                if (res.success) {
    ...
    const edit=(id)=>{
        history.push(`/addeditmasterskils/${id}`)
    }
